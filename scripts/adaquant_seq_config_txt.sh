...
python main.py --adaquant --optimize-weights --seq_adaquant --nbits_weight $nbits_weight --nbits_act $nbits_act  --num-sp-layers $num_sp_layers  --model $model -b 100 --evaluate results/$workdir/$model.absorb_bn.measure$perC_suffix --model-config "{'batch_norm': False,'measure': False, 'perC': $perC}" --dataset imagenet_calib --datasets-dir $datasets_dir -lpd "$prec_dict" --res-log adaquant.csv --cmp $cr
fi
done
